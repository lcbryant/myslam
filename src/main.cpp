...
int main (int argc, char** argv) {
  ...
  }else if(argc==2 && !strcmp(argv[1],"slam")){
    string mainpath,tmp,timepath;
    cout<<"请输入你想要生成视差图的文件夹"<<endl;
    cin>>mainpath;
    int n=0;
    timepath=mainpath+"/times.txt";
    ifstream fin(timepath.c_str());
    while(getline(fin,tmp))
    {
      n++;
    }
    cout<<"共有"<<n<<"份文件带转换"<<endl;
    for(int i=0;i<n;i++)
    {
      char s[7];
      sprintf(s,"%06d",i);
      string tl=mainpath+"/L/"+s+".pgm",tr=mainpath+"/R/"+s+".pgm";
